...
public class SecurityModuleImplOld extends SecurityServiceBase implements SecurityService {
    private static final Logger LOG = LoggerFactory.getLogger(SecurityModuleImplOld.class);

    private CurUserProviderOld curUserProvider;

    private CurUserProviderOld getCurUserProvider() {
        if(curUserProvider == null)
            curUserProvider = new CurUserProviderOld(this);
